 ...
 * 控制器抽象类
 */
public abstract class NewsBaseVideoPlayerController
        extends FrameLayout implements View.OnTouchListener {

    private Context mContext;
    protected IVideoPlayer mSHVideoPlayer;
    ...
    public NewsBaseVideoPlayerController(Context context) {
        super(context);
        mContext = context;
    ...
    public void setSHVideoPlayer(IVideoPlayer niceVideoPlayer) {
        mSHVideoPlayer = niceVideoPlayer;
    }
     ...
     *                  <ul>
     *                  <li>{@link NewsVideoPlayer#STATE_IDLE}</li>
     *                  <li>{@link NewsVideoPlayer#STATE_PREPARING}</li>
     *                  <li>{@link NewsVideoPlayer#STATE_PREPARED}</li>
     *                  <li>{@link NewsVideoPlayer#STATE_PLAYING}</li>
     *                  <li>{@link NewsVideoPlayer#STATE_PAUSED}</li>
     *                  <li>{@link NewsVideoPlayer#STATE_BUFFERING_PLAYING}</li>
     *                  <li>{@link NewsVideoPlayer#STATE_BUFFERING_PAUSED}</li>
     *                  <li>{@link NewsVideoPlayer#STATE_ERROR}</li>
     *                  <li>{@link NewsVideoPlayer#STATE_COMPLETED}</li>
     *                  </ul>
     */
     ...
     *                 <ul>
     *                 <li>{@link NewsVideoPlayer#MODE_NORMAL}</li>
     *                 <li>{@link NewsVideoPlayer#MODE_FULL_SCREEN}</li>
     *                 <li>{@link NewsVideoPlayer#MODE_TINY_WINDOW}</li>
     *                 </ul>
     */
    ...
    protected void startUpdateProgressTimer() {
        ...
        if (mUpdateProgressTimerTask == null) {
            mUpdateProgressTimerTask = new TimerTask() {
                @Override
                public void run() {
                    NewsBaseVideoPlayerController.this.post(new Runnable() {
                        @Override
                        public void run() {
                            updateProgress();
                        }
    ...
    public boolean onTouch(View v, MotionEvent event) {
        // 只有全屏的时候才能拖动位置、亮度、声音
        if (!mSHVideoPlayer.isFullScreen()) {
            return false;
        }
        // 只有在播放、暂停、缓冲的时候能够拖动改变位置、亮度和声音
        if (mSHVideoPlayer.isIdle()
                || mSHVideoPlayer.isError()
                || mSHVideoPlayer.isPreparing()
                || mSHVideoPlayer.isPrepared()
                || mSHVideoPlayer.isCompleted()) {
            hideChangePosition();
        ...
        switch (event.getAction()) {
            ...
            case MotionEvent.ACTION_MOVE:
                ...
                if (!mNeedChangePosition && !mNeedChangeVolume && !mNeedChangeBrightness) {
                    ...
                    if (absDeltaX >= THRESHOLD) {
                        ...
                        mGestureDownPosition = mSHVideoPlayer.getCurrentPosition();
                        ...
                        if (mDownX < getWidth() * 0.5f) {
                            ...
                            mGestureDownVolume = mSHVideoPlayer.getVolume();
                        }
                    }
                }
            ...
            case MotionEvent.ACTION_UP:
                if (mNeedChangePosition) {
                    mSHVideoPlayer.seekTo(mNewPosition);
                    hideChangePosition();
